...
type
  TfMilhar = class(TForm)
    ...
    procedure gPremiosDblClick(Sender: TObject);
  private
...
begin
  dm.rViewPrint.ProjectFile := 'teste.rav';
  dm.rViewPrint.Execute;
end;
...
begin
    ...
    with DM.carregarMilhar do begin
      SQL.Clear;
      Close;
      SQL.Add('SELECT * FROM '+txtTabela.Text+' t INNER JOIN clientes c ON');
      SQL.Add('c.idcliente = t.CLIENTEid');
      SQL.Add('WHERE t.CLIENTEid = "'+IntToStr(id_cliente)+'" ORDER BY MILHAR ASC');
      Open;
    end;
...
var
  tabelaClientes, tabelaVendedores, ListMilhar,ListPremios: TZQuery;
...
begin
  ...
  ListPremios := TZQuery.Create(nil);
  ...
  with ListMilhar do begin
    ...
    SQL.Add('t.CLIENTEID ORDER BY MILHAR ASC');
    Close;
    Open;
  end;
  ...
  with ListPremios do begin
    Connection :=  DM.conexao;
    Connection.Connect;
    SQL.Clear;
    Close;
    SQL.Add('SELECT * FROM premios p');
    SQL.Add('INNER JOIN login l ON l.idlogin = p.vendedorid');
    SQL.Add('where vendedorid = "'+IntToStr(id_vendedor)+'"');
    SQL.Add('ORDER BY ID DESC');
    Close;
    Open;
  end;
  DM.dsPremios.DataSet := ListPremios;
  gPremios.DataSource := DM.dsPremios;
  ...
  if (tabelaVendedores.RecordCount > 0) then begin
    ...
    if (dm.carregarPremios.RecordCount = 0) then begin
      Application.MessageBox('Nenhum premio foi encontrado para esse vendedor'+#13+'Por favor cadastre um premio para continuar','SKY PAINEL', MB_OK + MB_ICONWARNING);
      Abort;
    end;
    Application.MessageBox('Selecione o Premio pra continuar','SKY PAINEL', MB_OK + MB_ICONWARNING);
...
procedure TfMilhar.gPremiosDblClick(Sender: TObject);
begin
...
begin
  ...
  if ((cbClientesM.Text = '') OR (cbVendedoresM.Text = '')) then begin
    ...
    if (qrValidar.RecordCount > 0) then begin
         novaMilhar := TZQuery.Create(nil);
          with novaMilhar do begin
             ...
             SQL.Clear;
             Close;
             SQL.Add('SELECT * FROM '+txtTabela.Text+' t INNER JOIN clientes ON clientes.idcliente = t.clienteid where milhar = :m AND premioid = :premio');
             ParamByName('m').AsString := txtMilhar.Text;
             ParamByName('premio').AsInteger := dm.carregarPremios.FieldByName('ID').AsInteger;
             Open;
          end;
          ...
          if (MessageBox(Handle, PChar('Essa milhar já esta cadastra.'+#13+'Para: '+novaMilhar.FieldByName('nome_cliente').AsString+#13+'Voce deseja cadastra para o novo cliente? ('+cbClientesM.Text+')'), PChar('SKY PAINEL'), MB_YESNO or MB_ICONQUESTION) = 7) then begin
            ...
            with qrINSERT do begin
             ...
             Connection.Connect;
                SQL.Clear;
                Close;
                SQL.Add('UPDATE '+txtTabela.Text+' SET MILHAR = :m, CLIENTEID = :IDC, PREMIOID = :PR WHERE milhar = :MID');
                ParamByName('IDC').AsInteger := id_cliente;
                ParamByName('M').AsString := txtMilhar.Text;
                ParamByName('MID').AsString := txtMilhar.Text;
                ParamByName('PR').AsInteger := dm.carregarPremios.FieldByName('ID').AsInteger;
                ExecSQL;
           end;
            txtMilhar.Text := '';
            DM.dsMilhar.DataSet := DM.carregarMilhar;
    ...
    with qrMISELECIONADA do begin
       ...
       SQL.Clear;
       Close;
       SQL.Add('SELECT * FROM '+txtTabela.Text+' WHERE milhar = :m AND CLIENTEID = :v AND PREMIOID = :premio');
       ParamByName('m').AsString := txtMilhar.Text;
...
begin
    ...
    id_cliente := Integer( cbClientesM.Items.Objects[cbClientesM.ItemIndex ] );  //Pega o id do registro selecionado no combox
    Gauge1.Visible := True;
    Gauge1.MaxValue := StrToInt(txtTotalMilharSortida.Text);
    Gauge1.Progress := 0;
